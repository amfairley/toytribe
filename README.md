...
- **Password storage**: When the user changes or creates their password, the user entry is not saved as it is in the database. It is salted with an 8 byte string that precedes the password, ensuring that any 2 exact passwords are different. It is then hashed. The hashing is done using Password-Based Key Derivation Function 2 (PBKDF2) with Secure Hash Algorithm 256-bit (SHA-256) and the result is stored as the user password. This makes it computationally expensive to crack passwords, providing robust security. This is a one-way hash, making it very difficult to reverse engineer the user password from the stored value.
- **Usernames**: Originally first and last names were displayed on user reviews and user profiles, but this was changed to introduce and display unique usernames. This allows users to feel that their personal information is not being shared without their consent.
...
- Multiple default profile images to choose between
...
- [Jinja2](https://palletsprojects.com/p/jinja/)
    - A templating engine for python allowing template inheritance and injection of python variables and control structures into HTML pages.
...
- My tutor Paul for his guidance and teaching.
